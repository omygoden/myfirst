    ...
    private static function sendRequest($client, $url, $method, $body=null, $times=1) {
        ...
        if ($errorCode) {
            ...
            if ($times < $retries) {
                ...
                if ($errorCode === 28) {
//                    throw new \JPush\Exceptions\APIRequestException($msg . "Response timeout. Your request has probably be received by JPush Server,please check that whether need to be pushed again." );
                } elseif ($errorCode === 56) {
                // resolve error[56 Problem (2) in the Chunked-Encoded data]
//                    throw new \JPush\Exceptions\APIRequestException($msg . "Response timeout, maybe cause by old CURL version. Your request has probably be received by JPush Server, please check that whether need to be pushed again.");
                } else {
//                    throw new \JPush\Exceptions\APIRequestException("$msg . Connect timeout. Please retry later. Error:" . $errorCode . " " . curl_error($ch));
                }
            }
        } else {
    ...
    public static function processResp($response) {
        ...
        if (is_null($data)) {
//            throw new \JPush\Exceptions\ServiceNotAvaliable($response);
